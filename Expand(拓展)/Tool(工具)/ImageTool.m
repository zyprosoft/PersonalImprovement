...
+(UIImage *) imageCompressForSize:(UIImage *)sourceImage targetSize:(CGSize)size
{
    ...
    if(CGSizeEqualToSize(imageSize, size) == NO)//图片本身比例与希望压缩得到的比例不一致
    {
        CGFloat widthFactor = targetWidth / width;//宽度压缩比
        CGFloat heightFactor = targetHeight / height;//高度压缩比
        
        //下面语句功能是
        if(widthFactor > heightFactor)//宽度压缩比大，说明希望得到的宽度与图片本身宽度差别更小一些
        {
...
+(UIImage *) imageCompress:(UIImage *)sourceImage ForMaxWidth:(CGFloat)maxWidth AndMaxHeight:(CGFloat)maxHeight//对图片进行像素尺寸压缩，并且该图片的宽度不得大于maxWidth，高度不得大于maxHeight
{
    CGSize imageSize = sourceImage.size;
