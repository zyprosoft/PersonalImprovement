...
extension UIView{
    ...
    fileprivate func makeScaleValue() -> CGFloat{
        return 1 - 0.7 * (CGFloat(arc4random()%101 - 50)/CGFloat(50))
    }
    ...
    fileprivate func makeRandomPath(_ aLayer:CALayer) -> UIBezierPath{
        ...
        let controlPointOffSetY = layer.position.y - 0.2 * layer.frame.size.height - CGFloat(arc4random()%Int(1.2 * layer.frame.size.height))
        let endPointY = layer.position.y + layer.frame.size.height/2 + CGFloat(arc4random()%Int(layer.frame.size.height/2))
    ...
    open override class func initialize() {
        ...
        dispatch_once(&Static.token) {
            ...
            let swizzledSelector = #selector(UIView.WS_willMoveToSuperview(_:))
    ...
    func WS_willMoveToSuperview(_ newSuperView:UIView){
        removeBoomCells()
...
extension UIImage{
    ...
    func createARGBBitmapContextFromImage() -> CGContext{
        if aRGBBitmapContext != nil{
            ...
            let bitmapData = UnsafeMutableRawPointer.allocate(capacity: bitmapByteCount)
    ...
    func getPixelColorAtLocation(_ point:CGPoint) -> UIColor{
        ...
        let resData = UnsafePointer<UInt8>(cgctx.data)
