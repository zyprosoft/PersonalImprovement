...
@property (nonatomic,strong)OldAFHTTPRequestOperationManager *requestManager;
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.requestManager = [[OldAFHTTPRequestOperationManager alloc]initWithBaseURL:[NSURL URLWithString:@""]];
...
- (OldAFSecurityPolicy*)customSecurityPolicy
{
    /**** SSL Pinning ****/
...
- (void)cancelTaskThenWaitNextTime:(ZYNetWorkTask *)task
{
    dispatch_async(ZYNetworkManagerOperationQueue, ^{
        ...
        for (OldAFHTTPRequestOperation *operation in self.requestManager.operationQueue.operations ) {
...
- (void)addTask:(ZYNetWorkTask *)task{
    ...
    dispatch_async(ZYNetworkManagerOperationQueue, ^{
        ...
        OldAFHTTPRequestOperation *request = [[OldAFHTTPRequestOperation alloc]initWithRequest:urlRequest];
        if (task.requestMethod == ZYNetworkRequestMethodPOST) {
            request.securityPolicy = [self customSecurityPolicy];
        }
        
        [request setCompletionBlockWithSuccess:^(OldAFHTTPRequestOperation *operation, id responseObject) {
