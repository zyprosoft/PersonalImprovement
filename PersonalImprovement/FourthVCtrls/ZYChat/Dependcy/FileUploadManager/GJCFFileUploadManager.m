...
- (void)initClient
{
    ...
    self.requestOperationManager = [[OldAFHTTPRequestOperationManager alloc]initWithBaseURL:[NSURL URLWithString:self.defaultHostUrl]];
...
- (void)cancelTask:(NSString *)aTaskIdentifier shouldRemove:(BOOL)remove
{
    ...
    dispatch_async(_gjcfFileUploadManagerOperationQueue, ^{
        ...
        /* 推出请求 */
        [self.requestOperationManager.operationQueue.operations enumerateObjectsUsingBlock:^(OldAFHTTPRequestOperation *operation, NSUInteger idx, BOOL *stop) {
