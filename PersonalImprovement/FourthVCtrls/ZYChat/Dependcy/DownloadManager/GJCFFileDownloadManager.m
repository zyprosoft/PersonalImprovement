...
@property (nonatomic,strong)OldAFHTTPRequestOperationManager *requestOperationManager;
...
- (id)init
{
    if (self = [super init]) {
        ...
        self.requestOperationManager = [[OldAFHTTPRequestOperationManager alloc]initWithBaseURL:[NSURL URLWithString:@"http://www.zyprosoft.com"]];
    }
    return self;
}
...
- (void)setDefaultDownloadHost:(NSString *)host
{
    ...
    self.requestOperationManager = [[OldAFHTTPRequestOperationManager alloc]init];
...
- (void)addTask:(GJCFFileDownloadTask *)task
{
    ...
    dispatch_async(_gjcfFileDownloadManagerOperationQueue, ^{
        ...
        OldAFHTTPRequestOperation *downloadOperation = [[OldAFHTTPRequestOperation alloc]initWithRequest:downloadRequest];
        downloadOperation.userInfo = @{@"task": task};
        
        NSLog(@"GJCFFileDownloadManager 开始下载 地址:%@",downloadRequest.URL.absoluteString);
        
        /* 观察三个任务状态 */
        [downloadOperation setCompletionBlockWithSuccess:^(OldAFHTTPRequestOperation *operation, id responseObject) {
...
- (void)cancelTask:(NSString *)taskUniqueIdentifier
{
    ...
    dispatch_async(_gjcfFileDownloadManagerOperationQueue, ^{
        ...
        /* 退出任务 */
        for (OldAFHTTPRequestOperation *request in self.requestOperationManager.operationQueue.operations) {
