...
@interface LCChatKitExample () <XYDMWPhotoBrowserDelegate>
...
- (void)examplePreviewImageMessageWithInitialIndex:(NSUInteger)index allVisibleImages:(NSArray *)allVisibleImages allVisibleThumbs:(NSArray *)allVisibleThumbs {
    ...
    for (NSUInteger index = 0; index < allVisibleImages.count; index++) {
        ...
        if ([image_ isKindOfClass:[UIImage class]]) {
            photo = [XYDMWPhoto photoWithImage:image_];
        } else {
            photo = [XYDMWPhoto photoWithURL:image_];
        }
        [photos addObject:photo];
        
        id thumb_ = allVisibleThumbs[index];
        if ([thumb_ isKindOfClass:[UIImage class]]) {
            thumb = [XYDMWPhoto photoWithImage:thumb_];
        } else {
            thumb = [XYDMWPhoto photoWithURL:thumb_];
        }
    ...
    self.thumbs = thumbs;
    // Create browser
    XYDMWPhotoBrowser *browser = [[XYDMWPhotoBrowser alloc] initWithDelegate:self];
...
#pragma mark - XYDMWPhotoBrowserDelegate
