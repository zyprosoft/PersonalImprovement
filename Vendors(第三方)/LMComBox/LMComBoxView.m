...
//    titleLabel.frame = CGRectMake(2, 0, self.frame.size.width-imgW - 5 - 2, self.frame.size.height)
    ...
    [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self).with.insets(UIEdgeInsetsMake(0, 2, 0, imgW + 2));
    }];
    
    _arrow = [[UIImageView alloc]initWithFrame:CGRectZero];
//    _arrow.frame = CGRectMake(btn.frame.size.width - imgW - 2, (self.frame.size.height-imgH)/2.0, imgW, imgH);
...
//        make.top.equalTo(self.mas_top).with.offset(self.bottom);
//    }];
...
//            [_listTable setFrame:frame];
            [_listTable mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.equalTo(@0);
            }];
        } completion:^(BOOL finished){
    ...
    else
    {
        ...
        if (nil != keyWindow)
        {
            _backView = [[UIView alloc]initWithFrame:keyWindow.bounds];
            ...
            [UIView animateWithDuration:0.3 animations:^{
                ...
                //            }
                //            [_listTable setFrame:frame];
                [_listTable mas_remakeConstraints:^(MASConstraintMaker *make) {
                    make.left.equalTo(self);
                    make.right.equalTo(self);
                    make.top.equalTo(self.mas_bottom);
                    make.height.equalTo(@(_tableHeight>0?_tableHeight:tableH));
                }];
