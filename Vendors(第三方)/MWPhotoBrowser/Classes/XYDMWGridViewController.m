...
#import "XYDMWGridCell.h"
#import "XYDMWPhotoBrowserPrivate.h"
#import "XYDMWCommon.h"

@interface XYDMWGridViewController () {
...
@implementation XYDMWGridViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.collectionView registerClass:[XYDMWGridCell class] forCellWithReuseIdentifier:@"GridCell"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (visibleCells) {
        for (XYDMWGridCell *cell in visibleCells) {
...
- (void)adjustOffsetsAsRequired {
    ...
    if(self.isScrollToBottom){
        CGFloat margin = self.collectionView.contentSize.height-self.collectionView.bounds.size.height;
        CGFloat offset = margin > self.collectionView.contentOffset.y?margin:self.collectionView.contentOffset.y;
...
- (void)selectAllCell:(BOOL)isSelectAll {
    ...
    _isUnSelectAll = !isSelectAll;
    // 只会对cell非nil的生效
    for (int i = 0;i < [_browser numberOfPhotos];i ++) {
        XYDMWGridCell *cell = (XYDMWGridCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
        BOOL isCellSelected = [_browser photoIsSelectedAtIndex:i];
        if (_isSelectAll) {
            if (!isCellSelected) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    XYDMWGridCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GridCell" forIndexPath:indexPath];
    if (!cell) {
        cell = [[XYDMWGridCell alloc] init];
    }
    id <XYDMWPhoto> photo = [_browser thumbPhotoAtIndex:indexPath.row];
    cell.photo = photo;
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    [((XYDMWGridCell *)cell).photo cancelAnyLoading];
}
