...
@implementation XYDMWPhotoBrowser
...
- (void)_initialisation {
    ...
    // Listen for XYDMWPhoto notifications
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleXYDMWPhotoLoadingDidEndNotification:)
...
- (void)viewDidLoad {
    ...
    [self setToolBarAppearance:_toolbar];
    
    // Toolbar Items
    if (self.displayNavArrows) {
        NSString *arrowPathFormat = @"UIBarButtonItemArrow%@";
        UIImage *previousButtonImage = [UIImage imageForResourcePath:[NSString stringWithFormat:arrowPathFormat, @"Left"] ofType:@"png" inBundle:[NSBundle mainBundle]];
        UIImage *nextButtonImage = [UIImage imageForResourcePath:[NSString stringWithFormat:arrowPathFormat, @"Right"] ofType:@"png" inBundle:[NSBundle mainBundle]];
...
// 此代码我加，处理返回按钮的点击事件
- (void)clickBackBtn {
    if (_clickBackToExitBrowser) {
        [self.navigationController popViewControllerAnimated:YES];
    }else {
        if (!_gridController) {
            // 非grid样式点击返回只是回到grid样式
            [self showGrid:YES];
        }else {
            [self.navigationController popViewControllerAnimated:YES];
        }
...
- (void)performLayout {
    ...
    // Navigation buttons
    [self getNavigationLeftRightBarItem:YES];
    [self getNavigationLeftRightBarItem:NO];
    ...
    // Left button - Grid
    if ([self isShowGridBtn]) {
        hasItems = YES;
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"UIBarButtonItemGrid" ofType:@"png" inBundle:[NSBundle mainBundle]] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)]];
    } else {
    ...
    if (hideToolbar) {
        if ([self.delegate respondsToSelector:@selector(photoBrowserToolBarItems:)]) {
            NSArray *customItems = [self.delegate photoBrowserToolBarItems:self];
            if (customItems.count) {
                _toolbar.items = customItems;
...
// 此方法我分离出来
#pragma mark - 设置导航栏左上角、右上角按钮
- (void)getNavigationLeftRightBarItem:(BOOL)isLeft {
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        if (isLeft) {
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowserBackBarItem:)]) {
                newBackButton = [self.delegate photoBrowserBackBarItem:self];
            }else {
                // 此代码我改，增加了自定义的action，并添加了相应的属性
                newBackButton = [[UIBarButtonItem alloc] initWithTitle:backButtonTitle ? backButtonTitle : @"返回" style:UIBarButtonItemStylePlain target:self action:@selector(clickBackBtn)];
                // Appearance
            ...
            //此代码我加，如不加则点击返回时不会进入backBtnAction里面
            self.navigationItem.leftBarButtonItem = newBackButton;
        }else {
            if ([self.delegate respondsToSelector:@selector(photoBrowserRightTopBarItem:)]) {
                _doneButton = [self.delegate photoBrowserRightTopBarItem:self];
                self.navigationItem.rightBarButtonItem = _doneButton;
            }
        }
    }
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_viewHasAppearedInitially) {
        if (_autoPlayOnAppear) {
            XYDMWPhoto *photo = (XYDMWPhoto *)[self photoAtIndex:_currentPageIndex];
...
- (void)willMoveToParentViewController:(UIViewController *)parent {
    if (parent && _hasBelongedToViewController) {
        [NSException raise:@"XYDMWPhotoBrowser Instance Reuse" format:@"XYDMWPhotoBrowser instances cannot be reused."];
    }
}
...
#pragma mark - Tool Bar Appearance
...
- (void)layoutVisiblePages {
	...
	for (XYDMWZoomingScrollView *page in _visiblePages) {
...
- (XYDMWCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    XYDMWCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <XYDMWPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[XYDMWCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (void)loadAdjacentPhotosIfNecessary:(id<XYDMWPhoto>)photo {
    XYDMWZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <XYDMWPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
...
#pragma mark - XYDMWPhoto Loading Notification

- (void)handleXYDMWPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <XYDMWPhoto> photo = [notification object];
    XYDMWZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImage]) {
            // Successful load
...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
			XYDMWZoomingScrollView *page = [self dequeueRecycledPage];
			if (!page) {
				page = [[XYDMWZoomingScrollView alloc] initWithPhotoBrowser:self];
			}
            ...
            // Add caption
            XYDMWCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
            ...
            if (page.displayingVideo) {
                ...
                [playButton setImage:[UIImage imageForResourcePath:@"PlayButtonOverlayLarge" ofType:@"png" inBundle:[NSBundle mainBundle]] forState:UIControlStateNormal];
                [playButton setImage:[UIImage imageForResourcePath:@"PlayButtonOverlayLargeTap" ofType:@"png" inBundle:[NSBundle mainBundle]] forState:UIControlStateHighlighted];
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageForResourcePath:@"ImageSelectedOff" ofType:@"png" inBundle:[NSBundle mainBundle]] forState:UIControlStateNormal];
                ...
                if (self.customImageSelectedIconName) {
                    ...
                    selectedOnImage = [UIImage imageForResourcePath:@"ImageSelectedOn" ofType:@"png" inBundle:[NSBundle mainBundle]];
                }
...
- (void)updateVisiblePageStates {
    ...
    for (XYDMWZoomingScrollView *page in copy) {
...
- (XYDMWZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
	XYDMWZoomingScrollView *thePage = nil;
	for (XYDMWZoomingScrollView *page in _visiblePages) {
		if (page.index == index) {
			thePage = page; break;
		}
...
- (XYDMWZoomingScrollView *)dequeueRecycledPage {
	XYDMWZoomingScrollView *page = [_recycledPages anyObject];
	if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    id <XYDMWPhoto> currentPhoto = [self photoAtIndex:index];
...
- (CGRect)frameForGridCtrl {
    ...
    return CGRectOffset(frame, 0, (self.startOnGrid ? -1 : 1) * self.view.bounds.size.height);
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    // 此行代码我注释掉，会导致滑动式界面颤动，并且底部工具栏的动画也会有偏差(发现并不会了)
...
//	}
    ...
    XYDMWPhoto *photo = [self photoAtIndex:_currentPageIndex];
...
- (void)selectAllCell {
    if (_gridController) {
        ...
        [_gridController selectAllCell:_isSelectAll];
    }
}
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController = [[XYDMWGridViewController alloc] init];
    ...
    _gridController.isScrollToBottom = self.isScrollToBottom;
    _gridController.view.frame = [self frameForGridCtrl];
    ...
    [UIView animateWithDuration:animated ? 0.3 : 0 animations:^(void) {
        _gridController.view.frame = [self frameForBrowserView];
...
- (void)hideGrid {
    ...
    XYDMWGridViewController *tmpGridController = _gridController;
    _gridController = nil;
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        for (XYDMWZoomingScrollView *page in _visiblePages) {
            if (page.captionView) {
                XYDMWCaptionView *v = page.captionView;
...
- (void)actionButtonPressed:(id)sender {
    ...
    id <XYDMWPhoto> photo = [self photoAtIndex:_currentPageIndex];
